 ...
 *
 * @param tamanho numero de posicoes da fila
 */

	FilaCircular(int tamanho){

		municipios = new Municipio[tamanho+1];
 ...
 */
	...
	public void inserir(Municipio municipio){
		...
		if(((ultimo+1) % municipios.length == primeiro)){
			...
			try{
				...
				MyIO.println("Erro remover/inserir");

			}

		}

			municipios[ultimo] = municipio;
			ultimo = (ultimo + 1) % municipios.length;
 ...
 */
	...
	public void remover() throws Exception{
		...
		primeiro = (primeiro + 1) % municipios.length;
 ...
 */
	...
	public double mediaPopulacao() throws Exception{
		...
		while(i != ultimo){
			...
			media += municipios[i].getPopulacao();
			cont++;
			i = (i+1) % municipios.length;

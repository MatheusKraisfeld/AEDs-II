...
/**
 * Ordena a lista utilizando metodo de selecao recursivo
 * @param low ultimo elemento da parte ordenada
 * @param high ultimo elemento da parte desordenada
 */

	public void selecaoRecursiva(int low, int high){
		...
		if(low < high){
			...
			for(int i = low + 1; i <= high; i++){
				...
				if(municipios[i].getAtualizacaoPlano() == municipios[tmp].getAtualizacaoPlano() && municipios[i].getID() < municipios[tmp].getID()){
				...
				if(municipios[i].getAtualizacaoPlano() < municipios[tmp].getAtualizacaoPlano()){
			...
			Municipio novo = municipios[low].clone();
			municipios[low] = municipios[tmp].clone();
			municipios[tmp] = novo;
			this.mov += 3;
			selecaoRecursiva(low + 1, high);
...
class TP05Q04{
	...
	public static void main(String[] args){
		...
		list.selecaoRecursiva(0, list.n -1);
		...
		Arq.openWrite("matricula_selecaoRecursiva.txt", "utf-8");
		Arq.println("572251\t" + list.comp + "\t" + list.mov + "\t" + sec/10);
		Arq.close();

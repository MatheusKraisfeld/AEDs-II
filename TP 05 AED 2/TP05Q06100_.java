 ...
 */
	...
	public void insercaoPorCor(int cor, int h){
		...
		for(int i = (h + cor); i < n; i += h){
			...
			while(j >= 0 && compararNome(municipios[j].getUF(), tmp.getUF(), municipios[j], tmp)){
...
/**
 * Compara a UF de dois municipios,se for igual compara a ID
 * @param x UF do municipio[j]
 * @param y UF do municipio[i]
 * @param j Municipio[j]
 * @param i Municipio[i]
 * @return true para UF diferente, ou UF igual e ID maior. Qualquer outra situacao, false
 */

	//Metodo feito com a ajuda do aluno Ernesto
	public boolean compararNome(String x, String y, Municipio j, Municipio i){
		
		boolean resp = false;
		int comparar = x.compareTo(y);
		
		if(comparar > 0) resp = true;
		if(comparar == 0){

			if(j.getID() > i.getID()) resp = true;
			else resp = false;
...
class TP05Q06{
	...
	public static void main(String[] args){
		...
		Arq.openWrite("matricula_shellsort.txt", "utf-8");

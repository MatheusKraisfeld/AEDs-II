 ...
 */
	...
	public void quicksort(int esq, int dir){
		...
		int i = esq, j = dir, pivo = municipios[(dir + esq)/2].getPopulacao();
...
/**
 * Ordena a lista com o metodo countingsort
 * 
 */
	...
	public void countingsort(){
		...
		Municipio[] ordenado = new Municipio[n];
		...
		for(int i = 0; i < n; i++){
			...
			count[municipios[i].getNumeroFuncionarios()]++;
		...
		for(int i = n - 1; i >= 0; i--){
			...
			ordenado[count[municipios[i].getNumeroFuncionarios()]-1] = municipios[i].clone();
			count[municipios[i].getNumeroFuncionarios()]--;

		}

		//movendo os arquivos do array de saida para o array original
		for(int i = 0; i < n; i++){

			municipios[i] = ordenado[i];
...
/**
 * Encontra o elemento com o maior numero de funcionarios da lista
 * @return maior elemento 
 */

	public int getMaior(){

		int maior = municipios[0].getNumeroFuncionarios();

		for(int i = 0; i < n; i++){

			if(municipios[i].getNumeroFuncionarios() > maior){

				maior = municipios[i].getNumeroFuncionarios();

...
class ArvoreArvore{
	...
	ArvoreArvore(){
		...
		raiz = null;
		int array[] = {29, 21, 50, 14, 25, 35, 52, 12, 16, 23, 27, 32, 42, 51, 53, 11, 13, 15, 17, 22, 24, 26, 28, 31, 33, 41, 43};
	...
	public void pesquisar(int elemento){
		...
		mostrarCentral(elemento);

	}
	
    public void mostrarCentral(int elemento) {
        mostrarCentral(raiz, elemento);
    }
     ...
     * @param i No em analise.
     */
    private void mostrarCentral(No i, int elemento) {

	    boolean resp;
	    if (i != null) {

            mostrarCentral(i.esq, elemento); // Elementos da esquerda.

	    resp = i.outro.pesquisar(elemento);

	    //if(resp == true) return;

            mostrarCentral(i.dir, elemento); // Elementos da direita.
        }
    }
	...
	public No inserir(No raiz, int elemento){
	...
	public No inserir(No raiz, Municipio x){

		if(raiz == null){


		}
		else if(x.getCodigoUF() < raiz.elemento){
			...
			try{
				raiz.outro.inserir(x);
			}catch(Exception e){

				MyIO.println("Erro raiz.outro.inserir");
...
class TP07Q02{
	...
	public static void main(String[] args){
		...
		Arq.openWrite("572251_arvoreBinaria.txt", "utf-8");

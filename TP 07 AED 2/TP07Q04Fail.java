...
class Alvinegra {
     ...
     * <code>false</code> em caso contrario.
     */
    private boolean pesquisar(int elemento, Municipio i) {
      boolean resp;
        if (i == null) {
	 MyIO.println("NAO");
         resp = false;
 
      } else if (elemento == i.getID()) {
	 MyIO.println("SIM");
         resp = true;
...
//*****************************************************************************************************************************************************************************/ 
    ...
    public void inserir(Municipio elemento) throws Exception {
      ...
      //Senao, se a arvore tiver dois elementos (raiz e dir)
	    ...
	    raiz.esq = elemento.clone();
         ...
         } else if (raiz.dir.getID() > elemento.getID()){
      ...
      //Senao, se a arvore tiver dois elementos (raiz e esq)
	    ...
	    raiz.dir = elemento.clone();
         ...
         } else if (raiz.esq.getID() < elemento.getID()){
   ...
   private void balancear(Municipio bisavo, Municipio avo, Municipio pai, Municipio i){
 
      //Se o pai tambem e preto, reequilibrar a arvore, rotacionando o avo
      if(pai.cor == true){
 
         //4 tipos de reequilibrios e acoplamento
         if(pai.getID() > avo.getID()){ // rotacao a esquerda ou direita-esquerda
            if(i.getID() > pai.getID()){
               avo = rotacaoEsq(avo);
            } else {
     ...
     * @throws Exception Se o elemento existir.
     */
    private void inserir(Municipio elemento, Municipio bisavo, Municipio avo, Municipio pai, Municipio i) throws Exception {
        if (i == null) {
 
         if(elemento.getID() < pai.getID()){
            //i = pai.esq = new No(elemento, true);
	    i = pai.esq = elemento.clone();
	    pai.esq.cor = true;
         } else {
            //i = pai.dir = new No(elemento, true);
	    i = pai.dir = elemento.clone();
	    pai.dir.cor = true;
         }
        ...
        //Achou um 4-no: eh preciso fragmeta-lo e reequilibrar a arvore
         ...
         if (elemento.getID() < i.getID()) {
            inserir(elemento, avo, pai, i, i.esq);
         } else if (elemento.getID() > i.getID()) {
   ...
   private Municipio rotacaoDir(Municipio no) {
   ...
   private Municipio rotacaoEsq(Municipio no) {
   ...
   private Municipio rotacaoDirEsq(Municipio no) {
      no.dir = rotacaoDir(no.dir);
      return rotacaoEsq(no);
   }
 
   private Municipio rotacaoEsqDir(Municipio no) {
